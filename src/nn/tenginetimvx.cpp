...
bool TengineTimvx::init(const std::string &model, const std::string &cfg, void *params)
{
	...
	LOG("Trying TIMVX ..");
	context_t timvxContext = create_context("timvx", 1);
	if (timvxContext == nullptr)
	{
		LOGE("Create context TIMVX failed!");
		return false;
	}

	if (set_context_device(timvxContext, "TIMVX", nullptr, 0) != 0)
	{
		LOGE("Add context device TIMVX failed!");
		return false;
	}

	// Create graph, load tengine model xxx.tmfile
	_graph = create_graph(timvxContext, "tengine", model.c_str());
	if (_graph == nullptr)
	{
	...
	if (set_tensor_buffer(inputTensor, _inputData.data(), imgSize) < 0)
	{
	...
	get_tensor_quant_param(inputTensor, &_inputScale, &_inputZeroPoint, 1);
	return true;
}
