...
bool AvWriter::openFile(const string &fileName)
{
    ...
    AVOutputFormat *outFormat = const_cast<AVOutputFormat*>(av_guess_format(nullptr, fileName.c_str(), nullptr));
    if (!outFormat)
    {
    ...
    AVCodec *codec = const_cast<AVCodec*>(avcodec_find_encoder(outFormat->video_codec));
    if (!codec)
    {
